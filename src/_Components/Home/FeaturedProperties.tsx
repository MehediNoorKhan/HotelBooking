...
export default function FeaturedProperties({
  ...
  const mapAmenityToIcon = (amenityName: string) => {
        ...
        {visibleProperties.map((item, index) => (
          ...
          >
            ...
            <div className="relative w-full" style={{ height: "200px" }}>
              ...
              />
              {/* Price badge */}
              <div className="absolute top-2.5 right-2.5 bg-background px-2 py-1.5 rounded-full text-[14px] sm:top-3 sm:right-3 sm:text-[15px] md:text-[16px] xl:top-3 xl:right-3 xl:px-[9px] xl:py-[9px] xl:text-[16px]">
            ...
            <div className="px-3.5 pt-3 pb-[18px] sm:px-4 md:px-[18px] xl:px-4 xl:pt-4 xl:pb-5">
                  ...
                  return (
                    icon && (
                      <div key={a.id} className="flex items-center gap-2">
                        <img src={icon} className="w-4 h-4" alt={a.name} />
                        <p className="text-foreground/70 text-[14px]">{a.name}</p>
                      </div>
                    )
                  );
                })}
              ...
              <Link
                ...
                className="mt-3.5 w-full border border-primary/40 text-foreground py-2.5 rounded-xl text-[14px] sm:text-[15px] md:text-[16px] font-medium transition hover:bg-primary hover:text-muted xl:py-2 xl:px-[137px] cursor-pointer text-center block"
              >
                See details...
              </Link>
            </div>
